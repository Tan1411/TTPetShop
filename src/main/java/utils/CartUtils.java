...
public class CartUtils {
	public static List<DetailCart> getLstDetailCarts(Connection conn, int cartID ) throws SQLException {
		   ...
		   while (rs.next()) {
			   ...
			   ct.setFormatPriceString(String.format("%,d VNĐ",ct.getQuantity()*ct.getPro().getPrice()));
			   list.add(ct);
		   }
	...
	public static void changeStatusAll(Connection conn, int cartID, String status) {
		// TODO Auto-generated method stub
		try {
			String sql = "EXEC dbo.updateAllStatusForProInDetailCart @cartID = ? ,@status = ?";
			PreparedStatement pstm = conn.prepareStatement(sql);
			pstm.setInt(1, cartID);
			if(!status.isEmpty())
				pstm.setInt(2, Integer.parseInt(status) );
			pstm.executeUpdate();
			System.out.println("change All status of products in detailcart");
