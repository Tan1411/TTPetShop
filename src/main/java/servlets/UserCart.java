		...
		List<DetailCart> list = null;
		 try {
				...
				if(action != null && action.equalsIgnoreCase("checkout")){
					...
					if (action != null && action.equals("addDiscountID")) {
					...
					}else if (action != null && action.equals("changeStatusAll")) {
						String status = request.getParameter("statusAll");
						CartUtils.changeStatusAll(conn, us.getCartID().getCartID(), status);
					}
					else {
						...
						if (action.equalsIgnoreCase("changeStatus"))
						{
							...
							int price_item = Integer.parseInt(request.getParameter("price_item"));
							int newVal = Integer.parseInt(request.getParameter("newVal"));
							String data =  (String.format("%,d VNĐ", price_item*newVal));
